            ...
            var response = await ApiResponseHelper.ExecuteSafetyAsync<object>(async () =>
            {
                ...
                try
                {
                    ...
                    foreach (var item in dto.Items)
                    {
                        ...
                        {
                            var sql = "SELECT * FROM TON_KHO WITH (UPDLOCK, ROWLOCK) WHERE MaThuoc = {0} AND DonViTinh = {1} AND SoLuongCon > 0 ORDER BY HanSuDung";
        ...
        public async Task<IActionResult> CreateOnline([FromBody] HoaDonCreateDto dto)
        {
            var response = await ApiResponseHelper.ExecuteSafetyAsync<object>(async () =>
            {
                ...
                try
                {
                    ...
                    // For online orders we allocate lots and decrease inventory (same as Create)
                    foreach (var item in dto.Items)
                    {
